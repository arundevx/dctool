...
export default function ImageConvertClient({ fromFormat, toFormat }: ImageConvertClientProps) {
  ...
  const handleConvert = async () => {
    ...
    try {
      await downloadBlobFromApi(
        "/api/images/convert",
        formData,
        `converted_${file.name.split(".")[0]}.${toFormat.toLowerCase()}`
      );
    } catch (err) {
  ...
  return (
    <ToolLayout
      ...
      title={`Convert ${fromFormat} to ${toFormat}`}
      description={`Upload your ${fromFormat} image and instantly download it as ${toFormat}. Free and secure.`}
        ...
        <div className="p-8 dark:bg-white/5 bg-black/5 flex justify-center">
          ...
          >
            {loading ? (
              ...
              <>
                <Download className="h-5 w-5" /> Convert to {toFormat}
              </>
            )}
          </Button>
        </div>
