...
export default function WordToPdfClient() {
  ...
  const handleConvert = async () => {
    ...
    try {
      await downloadBlobFromApi("/api/pdfs/word-to-pdf", formData, "converted.pdf");
    } catch (err) {
  ...
  return (
    <ToolLayout
      breadcrumbs={toolBreadcrumbs(CATEGORY_META.pdf.label, CATEGORY_META.pdf.href, "Word to PDF")}
      title="Word to PDF"
      description="Convert your Word document (.docx) to a PDF file."
      icon={<FileOutput className="h-10 w-10 text-rose-500" />}
      colorTheme="rose"
      privacyMode="server"
      fileLimit="25 MB"
    >
      <div className="glass-panel rounded-3xl overflow-hidden">
        <PdfUploadZone
          ...
          label="Word (.docx)"
        />
        <p className="px-6 pb-2 text-sm text-muted-foreground text-center">Supports .docx files. Complex layouts may simplify during conversion.</p>
        ...
        <div className="p-8 flex justify-center border-t dark:border-white/5 border-black/5">
          <Button size="lg" className="h-14 px-10 rounded-full bg-gradient-to-r from-red-600 to-rose-600 text-white border-0 gap-2" onClick={handleConvert} disabled={loading || !file}>
            {loading ? <><Loader2 className="h-5 w-5 animate-spin" /> Converting...</> : <><Download className="h-5 w-5" /> Download PDF</>}
          </Button>
        </div>
      </div>
    </ToolLayout>
