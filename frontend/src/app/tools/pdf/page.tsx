...
import { FileText, Layers, Minimize2 } from "lucide-react";
import { constructMetadata } from "@/lib/seo";
import { ToolList } from "@/components/ui/tool-list";

export const metadata = constructMetadata({
  title: "PDF Tools",
  description: "Free online PDF tools to merge, compress, and split your documents securely.",
  path: "/tools/pdf",
});
...
const tools = [
  ...
  },
  {
    ...
    icon: <Minimize2 className="h-6 w-6 text-red-500" />,
...
export default function PdfToolsPage() {
  return (
    <div className="w-full min-h-screen grid-pattern relative">
      <div className="absolute top-0 left-0 w-full h-[400px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-rose-500/10 via-background to-transparent pointer-events-none -z-10"></div>
      ...
      <div className="container mx-auto py-16 px-4 md:px-8 relative z-10">
        ...
        <ToolList tools={tools} colorTheme="rose" />
      </div>
    </div>
  );
}
